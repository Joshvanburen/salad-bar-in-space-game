...
static void interpret_ir_data(struct wiimote_t* wm) {
	...
	#ifdef _DEBUG_
	DEBUG("IR visible dots: %i", wm->ir.num_dots);
	for (i = 0; i < 4; ++i)
		if (dot[i].visible)
			DEBUG("IR[%i][order %i] (%.3i, %.3i) -> (%.3i, %.3i)", i, dot[i].order, dot[i].rx, dot[i].ry, dot[i].x, dot[i].y);
	DEBUG("IR[absolute]: (%i, %i)", wm->ir.x, wm->ir.y);
	#endif
}
