 ...
 *	@param unids		An array of unique identifiers for each wiimote.
 *	@param event_cb		Callback to be executed on an event.
 *	@param stat_cb		Callback to be executed on an status event.
 *	@param dis_cb		Callback to be executed on a disconnection event.
 *
...
int wiiuse_set_report_type(struct wiimote_t* wm) {
	...
	DEBUG("Setting report type: 0x%x", buf[1]);
 ...
 *	finishes.
 */
DLLEXPORT int wiiuse_read_data(struct wiimote_t* wm, wiiuse_read_cb read_cb, byte* buffer, unsigned int addr, unsigned short len) {
	...
	if (!wm->read_req) {
		...
		DEBUG("Added pending data read request.");
	}
...
void wiiuse_send_next_pending_read_request(struct wiimote_t* wm) {
	...
	*(short*)(buf + 4) = BIG_ENDIAN_SHORT(req->size);
	...
	DEBUG("Request read at address: 0x%x  length: %i", req->addr, req->size);
...
DLLEXPORT void wiiuse_status(struct wiimote_t* wm) {
	...
	DEBUG("Requested wiimote status.");
...
DLLEXPORT int wiiuse_write_data(struct wiimote_t* wm, unsigned int addr, byte* data, byte len) {
	...
	DEBUG("Writing %i bytes to memory location 0x%x...", len, addr);
...
DLLEXPORT void wiiuse_set_ir(struct wiimote_t* wm, int status) {
	...
	if (!status) {
		DEBUG("Disabled IR cameras for wiimote id %i.", wm->unid);
		wiiuse_set_report_type(wm);
		return;
	}
	...
	wiiuse_write_data(wm, WM_REG_IR_BLOCK1, (byte*)WM_IR_BLOCK1_CLIFF, sizeof(WM_IR_BLOCK1_CLIFF)-1);
	wiiuse_write_data(wm, WM_REG_IR_BLOCK2, (byte*)WM_IR_BLOCK2_CLIFF, sizeof(WM_IR_BLOCK2_CLIFF)-1);
	...
	DEBUG("Enabled IR camera for wiimote id %i.", wm->unid);
}
