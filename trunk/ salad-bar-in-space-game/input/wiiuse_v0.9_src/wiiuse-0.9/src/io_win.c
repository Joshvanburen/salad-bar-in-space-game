...
DLLEXPORT int wiiuse_find(struct wiimote_t** wm, int max_wiimotes, int timeout) {
	GUID device_id;
	HANDLE dev;
	...
	for (;; ++index) {
		...
		if ((attr.VendorID == WM_VENDOR_ID) && (attr.ProductID == WM_PRODUCT_ID)) {
			...
			INFO("Found wiimote [assigned wiimote id %i].", wm[found]->unid);
...
DLLEXPORT int wiiuse_connect(struct wiimote_t** wm, int wiimotes) {
	int connected = 0;
	int i = 0;
...
DLLEXPORT void wiiuse_disconnect(struct wiimote_t* wm) {
...
int wiiuse_io_read(struct wiimote_t* wm) {
	...
	if (!ReadFile(wm->dev_handle, wm->event, sizeof(wm->event), &b, &wm->hid_overlap)) {
		/* partial read */
		b = GetLastError();
		...
		r = WaitForSingleObject(wm->hid_overlap.hEvent, 10);
		if (r == WAIT_TIMEOUT) {
			...
			WARNING("A wait error occured on reading from wiimote %i.", wm->unid);
			return 0;
		}
...
int wiiuse_io_write(struct wiimote_t* wm, byte* buf, int len) {
	...
	switch (wm->stack) {
		case WIIUSE_STACK_UNKNOWN:
		{
			...
			ERROR("Unable to determine bluetooth stack type.");
			return 0;
		}
