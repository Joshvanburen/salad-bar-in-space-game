...
int main(){
	...
	ScriptManager::getSingleton().registerAsGlobal("void PrintString(string &str)", ::asFUNCTION(PrintString));

	ScriptManager::getSingleton().loadScript("test");


	Scripting::ScriptFunction* function = new Scripting::ScriptFunction();

	ScriptManager::getSingleton().registerScriptFunction("test_main", function);

	Scripting::ScriptFunction& newFunction = ScriptManager::getSingleton().getScript("test").addFunction("test_main");

	newFunction.callFunction();
