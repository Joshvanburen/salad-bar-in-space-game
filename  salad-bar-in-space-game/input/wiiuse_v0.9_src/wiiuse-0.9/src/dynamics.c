...
void calculate_orientation(struct accel_t* ac, struct vec3b_t* accel, struct orient_t* orient, int smooth) {
	...
	xg = (float)ac->cal_g.x - (float)ac->cal_zero.x;
	yg = (float)ac->cal_g.y - (float)ac->cal_zero.y;
	zg = (float)ac->cal_g.z - (float)ac->cal_zero.z;
	...
	if (accel->x <= ac->cal_g.x) {
		...
		if (smooth) {
			...
			if (((ac->st_roll < 0) && (orient->roll > 0)) || ((ac->st_roll > 0) && (orient->roll < 0))) {
				...
				orient->roll = ac->st_roll + (ac->st_alpha * (orient->roll - ac->st_roll));
				ac->st_roll = orient->roll;
			}
		}
	}

	if (accel->y <= ac->cal_g.y) {
		/* pitch */
		int s = 1;
		...
		if (smooth) {
			...
			if (((ac->st_pitch < 0) && (orient->pitch > 0)) || ((ac->st_pitch > 0) && (orient->pitch < 0))) {
				...
				orient->pitch = ac->st_pitch + (ac->st_alpha * (orient->pitch - ac->st_pitch));
				ac->st_pitch = orient->pitch;
			}
		}
	}
